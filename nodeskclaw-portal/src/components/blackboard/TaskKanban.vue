...
import { AlertCircle, Archive, CheckCircle2, Clock, DollarSign, Loader2, Play } from 'lucide-vue-next'
import { useWorkspaceStore, type TaskInfo } from '@/stores/workspace'
import { useI18n } from 'vue-i18n'

type ActiveTaskStatus = 'pending' | 'in_progress' | 'done' | 'blocked'
type TaskBucketKey = ActiveTaskStatus | 'history'
...
const historyBucket = computed(() => taskBuckets.history)
const showHistorySection = computed(() => {
  const state = historyBucket.value
  return state.loading || state.total > 0 || state.items.length > 0
})
...
function isValueEditable(bucketKey: TaskBucketKey, task: TaskInfo) {
  return bucketKey !== 'history' && task.status === 'done'
}
...
async function loadBucket(bucketKey: TaskBucketKey, reset = false) {
  ...
  try {
    const response = bucketKey === 'history'
      ? await store.fetchTasksPaginated(props.workspaceId, {
          bucket: 'inactive',
          page: nextPage,
          pageSize: PAGE_SIZE,
        })
...
<template>
  <div class="space-y-4">
    ...
    <div
      ...
      class="rounded-xl border border-border/50 bg-muted/10"
    >
      <div class="flex items-center gap-1.5 border-b border-border/40 px-3 py-2.5">
        <Archive class="h-3.5 w-3.5 text-muted-foreground" />
        <span class="text-xs font-medium">{{ t('blackboard.taskHistory') }}</span>
        <span class="text-xs text-muted-foreground">({{ historyBucket.total }})</span>
      </div>

      <div
        :ref="el => setScrollContainer('history', el)"
        class="h-[13rem] overflow-y-auto px-3 pb-3"
        @scroll.passive="handleBucketScroll('history', $event)"
      >
        <div class="grid grid-cols-3 gap-3 pt-3">
          <div
            v-for="task in historyBucket.items"
            :key="task.id"
            class="space-y-1.5 rounded-lg border border-border/50 bg-background/80 p-2.5 text-xs"
          >
            <div class="flex items-start justify-between gap-1">
        ...
        <div v-if="isInitialLoading('history')" class="flex justify-center py-6">
          <Loader2 class="h-5 w-5 animate-spin text-muted-foreground" />
        </div>

        <div v-else-if="historyBucket.items.length === 0" class="py-6 text-center text-xs text-muted-foreground">
          {{ t('blackboard.noTasks') }}
        </div>

        <div v-if="historyBucket.loading && historyBucket.items.length > 0" class="flex justify-center py-2">
          <Loader2 class="h-4 w-4 animate-spin text-muted-foreground" />
        </div>
      </div>
    </div>
